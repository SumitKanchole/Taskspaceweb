...
const fetchMyTasks = async () => {
  ...
  const res = await fetch(`${baseUrl}/api/tasks/my`, {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">
          <div className="p-2 bg-[#5c42bd]/20 rounded-lg">
            <CheckSquare className="w-6 h-6 text-[#7359dc]" />
          </div>
          My Tasks
        </h1>
        <p className="text-white/50 mt-2">Tasks you created in your own workspaces.</p>
      </div>
      ...
      {isLoading ? (
        <div className="flex-1 flex items-center justify-center min-h-[40vh] animate-in fade-in duration-700">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-white/50 font-medium animate-pulse">Loading your tasks...</p>
          </div>
        </div>
      ) : isError ? (
        ...
        <div className="bg-[#1e0e3b] rounded-2xl border border-white/5 shadow-lg overflow-hidden flex flex-col items-center justify-center p-12 text-center">
          <div className="w-24 h-24 mb-6 relative">
            <div className="absolute inset-0 bg-[#5c42bd]/10 rounded-full blur-xl" />
            <CheckSquare className="w-full h-full text-[#5c42bd]/40" strokeWidth={1} />
          </div>
          <h3 className="text-xl font-bold text-white mb-2">No tasks found</h3>
          <p className="text-white/40 text-sm max-w-[300px]">
            You haven't created any tasks in your workspaces yet.
          </p>
        </div>
      ) : (
              ...
              <Link key={task.id} href={`/tasks/${task.workspaceId}/${task.id}`}>
                <div className="p-5 flex items-center gap-4 hover:bg-white/[0.02] transition-colors cursor-pointer group">
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    <Badge variant="outline" className="bg-[#0b0f19] border-white/10 text-white/60 capitalize">
                      {task.priority}
                    </Badge>
